...
A data marketplace contains a list of pre-defined adapters (which can be rather generic by selecting a protocol and a data format or quite specific as, e.g., a Twitter source).
These adapters can be configured according to your needs and directly started on a set of worker nodes.

StreamPipes Connect also includes cool new features that make it even easier to connect sources: Based on machine learning models, we'll automatically recognize data characteristics (e.g., whether your event produces temperature or pressure data).
These characteristics will be directly added to the semantic model that acts in the background to support you during pipeline creation. In addition, pre-processing features will be integrated that will support edge computing use cases.
